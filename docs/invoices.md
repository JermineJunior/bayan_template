...
بعد نجاح الحفظ، تُستدعى `syncMaintenanceExpense()` لتحديث **مصروف الصيانة**
(انظر [expenses.md](expenses.md)): يزداد ليصبح `labor_cost` الخاص بأمر الصيانة +
مجموع إجمالي جميع فواتير الصرف (`qty × price`). المصروف نفسه أُنشئ مسبقًا عند
إنشاء أمر الصيانة (`MaintenanceObserver`) بقيمة `labor_cost` فقط، وهنا يُحدَّث
عندما تُربط قطع الغيار بأمر الصيانة عبر الفاتورة.
