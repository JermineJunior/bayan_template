...
> المرتبطة به. مصروف الصيانة يُسجَّل **فور إنشاء** الأمر بقيمة `labor_cost`
> (عبر `MaintenanceObserver`)، ثم يُحدَّث عند إضافة قطع غيار ليصبح
> `labor_cost + مجموع إجمالي الفواتير`. راجع [invoices.md](invoices.md) و
> [expenses.md](expenses.md). في قائمة أوامر الصيانة تُعرض «تكلفة قطع غيار +
> صيانة» = `labor_cost + Σ total الفواتير` (القطع فقط عند عدم وجود فواتير).
...
- **المصروفات (Expenses):** يُسجَّل مصروف صيانة تلقائي فور الإنشاء بقيمة
  `labor_cost`، ويُحدَّث عند إضافة فواتير ليصبح `labor_cost + إجمالي الفواتير`.
  راجع [expenses.md](expenses.md).
