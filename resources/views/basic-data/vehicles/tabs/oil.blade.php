@php
    $formatRemaining = function (\App\Models\VehicleOilChange $change): string {
...
<div x-show="tab === 'oil'" x-cloak role="tabpanel">
    <div class="space-y-6">
        {{-- Current oil status per oil type. --}}
        <div class="rounded-xl border border-border bg-surface p-6 shadow-sm">
            <div class="mb-4 flex flex-wrap items-center justify-between gap-4">
                ...
                @can('oil-changes.create')
                    <a
                        href="{{ route('vehicles.oil-changes.create', $vehicle) }}"
            ...
            @if ($oilStatus->isNotEmpty())
                <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                    @foreach ($oilStatus as $change)
                            ...
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0">
                                    ...
                                    <p class="mt-0.5 text-xs text-muted-foreground">
                                        {{ config('oil_types.'.$change->oil->oil_type, $change->oil->oil_type) }}
                                    </p>
                                </div>
            ...
            @else
                <p class="text-sm text-muted-foreground">
                    لا توجد زيوت مسجلة لهذه المركبة بعد.
                </p>
            @endif
        </div>
        ...
        <div class="rounded-xl border border-border bg-surface p-6 shadow-sm">
            ...
            @if ($vehicle->oilChanges->isNotEmpty())
            ...
            @else
                <p class="text-sm text-muted-foreground">
                    لا يوجد سجل تغيير زيوت لهذه المركبة.
                </p>
            @endif
        </div>
    </div>
