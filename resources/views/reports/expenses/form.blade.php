    ...
    <div class="mx-auto max-w-7xl px-4 py-8">
        <div class="mb-6">
            ...
            <p class="mt-1 text-sm text-muted-foreground">
                حدد المركبة أو نوع المصروف أو الفترة الزمنية لعرض المصروفات.
            </p>
        </div>

        <form
            method="GET"
            action="{{ route('reports.expenses.results') }}"
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach ($expenseTypeLabels as $value => $label)
                            <option value="{{ $value }}" @selected(request('expense_type') === $value)>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
